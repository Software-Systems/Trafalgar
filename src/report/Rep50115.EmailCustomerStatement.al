report 50115 EmailCustomerStatement
{
    Caption = 'Trafalgar - Customer Statement';
    DefaultLayout = RDLC;
    RDLCLayout = '.\Reports\Layouts\EmailCustomerStatement.rdl';
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            dataitem("Integer"; "Integer")
            {
                ...
                column(TodayFormatted; Format(Today, 0, '<Day,2> <Month Text,3> <Year4>'))
                {
                }
                ...
                column(CustomerABN; Customer.ABN)
                {
                }
                column(OurABNCaption; OurABNCaption)
                {
                }
                column(OurABNNumber; CompanyInfo.ABN)
                {
                }
                column(AccountNumberCaption; AccountNumberCaption)
                {
                }
                column(EnquiryCaption; EnquiryCaption)
                {
                }
                column(ActivityCaption; ActivityCaption)
                {
                }
                column(ReferenceCaption; ReferenceCaption)
                {
                }
                column(InvoiceAmountCaption; InvoiceAmountCaption)
                {
                }
                column(PaymentsCaption; PaymentsCaption)
                {
                }
                column(BalanceAUDCaption; BalanceAUDCaption)
                {
                }
    ...
    requestpage
    {
        ...
        begin
            ...
            StatementStyle := StatementStyle::"Open Item";
            UpdateReqPageParameters();
        end;
    }
    ...
    protected var
        ...
        OurABNCaption: Label 'ABN';
        AccountNumberCaption: Label 'Account Number';
        EnquiryCaption: Label 'Enquiries';

        ActivityCaption: Label 'Activity';
        ReferenceCaption: Label 'Reference';
        InvoiceAmountCaption: Label 'Invoice Amount';
    ...
    NewPrintUnappliedEntries: Boolean; NewIncludeAgingBand: Boolean;
    NewPeriodLength: Text[30]; NewDateChoice: Option "Due Date","Posting Date";
    NewLogInteraction: Boolean; NewStartDate: Date; NewEndDate: Date; ParPrintOnlyOpen: Boolean)
    begin
        InitRequestPageDataInternal();
